...
node("connect-bee", "CONNECT BEE", "frontend", "팜커넥트",
     ...
     "수정벌 활동량과 벌통 센서 데이터를 확인하는 하이브리드 앱과 웹뷰를 개발했습니다. 내·외부 온도, 습도, CO2를 시각화하고 팬·열선·쿨러 제어 UI를 같은 화면에 두어, 상태 확인과 조치 사이를 오가지 않아도 되게 했습니다. CES 2025 혁신상을 받은 제품이며, 하이브리드 앱 웹뷰 개발과 포스터 이미지 디자인을 맡았습니다.",
     "CONNECT BEE 하이브리드 앱",
     P(role="프론트엔드 개발 · 기획 · 디자인 · CES 2025 제출용 홍보 에셋 제작",
...
for reg, ang in AREA_ANGLE.items():
    ...
    for j, kid in enumerate(kids):
        ...
        kr = AREA_R[reg] + 2.05 + (0.55 if j % 2 else 0.0)
